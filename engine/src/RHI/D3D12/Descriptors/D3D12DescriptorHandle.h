...
class D3D12DescriptorHandle
{
  public:
	...
	const D3D12_CPU_DESCRIPTOR_HANDLE& GetCPU() const noexcept
	{
		return m_cpuHandle;
	}
	// Returns the GPU descriptor handle for shader-visible heaps.
	const D3D12_GPU_DESCRIPTOR_HANDLE& GetGPU() const noexcept
	{
		return m_gpuHandle;
	}
	...
	void SetIndex(UINT idx) noexcept
	{
		m_index = idx;
	}
	bool IsValid() const noexcept
	{
		return (m_index != InvalidIndex) && (m_cpuHandle.ptr != 0);
	}
	bool IsShaderVisible() const noexcept
	{
		return (m_heapType == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) ||
