...
void LinearAllocator::Initialize(uint64_t capacity, const wchar_t* debugName)
{
    ...
    HRESULT hr = GD3D12Rhi.GetDevice()->CreateCommittedResource(&heapProps,
...
LinearAllocation LinearAllocator::Allocate(uint64_t size, uint64_t alignment)
{
    ...
    do
    {
        ...
        currentOffset, newOffset, std::memory_order_acq_rel, std::memory_order_relaxed));
    ...
    while (newOffset > currentHigh &&
           !m_HighWaterMark.compare_exchange_weak(currentHigh, newOffset, std::memory_order_relaxed))
        ;
