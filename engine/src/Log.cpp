...
namespace
{
    ...
    public:
        ...
        void Append(const char* s) noexcept { if (s) Append(s, std::strlen(s)); }
        ...
        void Format(const char* fmt, Args... args) noexcept
        {
            ...
            if (n > 0) m_pos += (std::min)(static_cast<std::size_t>(n), space);
        }
        ...
        void Newline() noexcept { if (m_pos < kCapacity) m_data[m_pos++] = '\n'; }
