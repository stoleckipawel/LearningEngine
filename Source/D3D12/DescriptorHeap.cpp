...
void FDescriptorHeap::Create(D3D12_DESCRIPTOR_HEAP_TYPE Type, UINT NumObjectsPerFrame, D3D12_DESCRIPTOR_HEAP_FLAGS flags, LPCWSTR Name)
{
	this->NumObjectsPerFrame = NumObjectsPerFrame;
	heapDesc.NumDescriptors = NumObjectsPerFrame * BufferingCount;
	heapDesc.Type = Type;
...
D3D12_CPU_DESCRIPTOR_HANDLE FDescriptorHeap::GetCPUHandle(UINT FrameIndex, UINT Index)
{
	...
	handle.ptr = descriptorSize * (FrameIndex * NumObjectsPerFrame + Index) + handle.ptr;
	return handle;
}
